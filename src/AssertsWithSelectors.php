...
trait AssertsWithSelectors
{
    public function setupAssertsWithSelectors(): void
    {
        TestResponse::macro(
            'getSelectorContents',
            function (string $selector): DOMNodeList {
                $dom = new DOMDocument();
        ...
        TestResponse::macro(
            ...
            function (string $selector): TestResponse {
        ...
        TestResponse::macro(
            ...
            function (string $selector, string $value): TestResponse {
                $selectorContents = $this->getSelectorContents($selector);
        ...
        TestResponse::macro(
            ...
            function (string $selector, string $attribute): TestResponse {
                $selectorContents = $this->getSelectorContents($selector);
         ...
         */
        TestResponse::macro(
            'assertSelectorAttributeDoesNotExist',
            function (string $selector, string $attribute): TestResponse {
                $selectorContents = $this->getSelectorContents($selector);
        ...
        TestResponse::macro(
            ...
            function (string $selector, string $attribute, string $value): TestResponse {
                ...
                PHPUnit::fail(
                    "Attribute '{$attribute}' on selector '{$selector}' does not have the expected value '{$value}'."
                );
            }
        );
         ...
         */
        TestResponse::macro(
            'assertSelectorAttributeDoesNotEqual',
            function (string $selector, string $attribute, string $value): TestResponse {
                $this->assertSelectorAttributeExists($selector, $attribute);
                ...
                foreach ($selectorContents as $element) {
                    if ($element->getAttribute($attribute) === $value) {
                        PHPUnit::fail(
                            "Selector '{$selector}' found with attribute '{$attribute}' set to value '{$value}'."
                        );
                    }
                }
